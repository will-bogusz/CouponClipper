...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const url = `https://api.scrapfly.io/scrape?retry=false&tags=player%2Cproject%3Adefault&session=${sessionName}&proxy_pool=public_residential_pool&country=us&lang=en&asp=true&render_js=true&js_scenario=W3sid2FpdCI6MzAwfV0&key=${apiKey}&url=https%3A%2F%2Fstores.foodlion.com%2Fva%2Fblacksburg%2F1413-n.-main-street`;
  try {
  ...
  if (responseCode === "SUCCESS") {
    if (mode === 'validate') {
      ...
      //   {
      //       "scroll": {
      //           "selector": ".social-media-callouts"
      //       }
      //   },
      //   {
      //       "wait": 1200
      ...
      //   {
      //       "click": {
      //           "selector": "li:nth-of-type(1) .bg-white.border-2.border-dashed.cursor-pointer.font-semibold.h-\[4\.4rem\].item-details_color.rounded-brand.select-none.text-center.w-full",
      //           "multiple": false,
      ...
      const scenario = encodeURIComponent(Buffer.from(JSON.stringify(jsScenario)).toString('base64'));
      ...
      const clipUrl = `https://api.scrapfly.io/scrape?retry=false&tags=player%2Cproject%3Adefault&timeout=120000&session=${sessionName}&debug=true&proxy_pool=public_residential_pool&render_js=true&rendering_wait=3000&js_scenario=${scenario}&country=us&lang=en&asp=true&key=${apiKey}&url=https%3A%2F%2Ffoodlion.com%2Fsavings%2Fcoupons%2Fbrowse`;
      ...
      try {
        await attemptFetch(clipUrl);
      } catch (error) {
        console.error('Error fetching initial data:', error);
        res.status(200).json({ status: 'failure', message: 'Failed to load and click coupons' });
        return;
      }

      res.status(200).json({ status: 'success', message: `Scraping and coupon redemption successful.` });
      return;
    }
